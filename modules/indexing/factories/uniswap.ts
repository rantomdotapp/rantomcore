...
export default class UniswapFactoryIndexing extends BaseFactoryIndexing {
  ...
  protected getTopics(): Array<string> {
    if (this.config.version === 'univ2') {
      return [UniswapEventSignatures.PairCreated];
    } else {
      return [UniswapEventSignatures.PoolCreated];
    }
  }
  ...
  public async indexHistoricalData(): Promise<void> {
    ...
    while (startBlock <= latestBlock) {
      ...
      for (const log of logs) {
        const liquidityPool = await this.getLiquidityPoolConstant(log);
        if (liquidityPool) {
          operations.push({
            updateOne: {
              filter: {
